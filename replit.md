...
- **Navigation wait strategy** — uses `waitUntil: 'load'` (not `networkidle2`) for both `navigate()` and `submitForm()` to avoid hanging on heavy SIGAA pages with continuous network activity (analytics, clocks, AJAX).
- **Race condition prevention** — `waitForNavigation` promise is created BEFORE `form.submit()` to avoid missing fast navigation events.
